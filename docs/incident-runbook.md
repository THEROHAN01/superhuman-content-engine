...
| Everything rejected by the gate with `SYNTHETIC_EVIDENCE`   | `RESEARCH_PROVIDER=mock` - by design                      | configure a real research provider (`fixture` for a demo), or use ideas that do not require evidence                                                                                                             |
| A learning event is stuck in `failed`                       | the model was unreachable when it was processed           | `POST /learning-events/:id/process` once it is back; `failed` is recoverable, and a retry cannot create a second atom                                                                                            |
...
| Publication stuck `retry_pending`                           | provider erroring transiently                             | `POST /content-items/:id/schedule` for the same slot retries through the same row and idempotency key, or wait for the hourly `POST /system/sweep`; after the attempt budget it becomes `failed` with the reason |
